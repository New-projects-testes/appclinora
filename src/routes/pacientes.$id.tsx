...
import {
  ...
  Save, Plus, Clock, CircleDollarSign, ShieldCheck, Pencil, FileSearch,
} from "lucide-react";
...
const fmtDate = (iso: string) => new Date(iso).toLocaleDateString("pt-BR", { day: "2-digit", month: "long", year: "numeric" });
const fmtTime = (iso: string) => new Date(iso).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" });

export const Route = createFileRoute("/pacientes/$id")({ component: PatientDetail });
...
function PatientDetail() {
  ...
  const { data: patientSessions = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("sessions").select("*").eq("patient_id", id)
  ...
  const updateStatusMut = useMutation({
    mutationFn: async (status: PatientStatus) => {
  ...
  const updatePatientMut = useMutation({
    mutationFn: async (form: Partial<PatientRow>) => {
  ...
  const updateSessionMut = useMutation({
    mutationFn: async ({ sessionId, updates }: { sessionId: string; updates: Partial<SessionRow> }) => {
  ...
  const insertSessionMut = useMutation({
  ...
  const upcoming = useMemo(
    () => [...patientSessions].reverse().find((s) => s.status === "scheduled" && new Date(s.date_time) > new Date()),
    [patientSessions]
  );
  ...
  const fin = useMemo(() => {
    ...
    return {
      paidTotal: paid.reduce((a, s) => a + Number(s.value), 0),
      pendingTotal: pending.reduce((a, s) => a + Number(s.value), 0),
      isentoTotal: isento.reduce((a, s) => a + Number(s.value), 0),
  ...
  if (isLoading) {
    return <AppShell><div className="p-10 text-sm text-muted-foreground">Carregando...</div></AppShell>;
  }
  ...
  const saveNotes = async () => {
    ...
    if (upcoming) {
      await updateSessionMut.mutateAsync({
        ...
        updates: { status: "done", notes: draft },
      });
    } else {
      await insertSessionMut.mutateAsync({
        owner_id: user.id,
        patient_id: id,
    ...
    <AppShell>
      <div className="px-6 md:px-10 py-8 max-w-6xl">
        <Breadcrumb className="mb-5">
          <BreadcrumbList>
            <BreadcrumbItem><BreadcrumbLink asChild><Link to="/pacientes">Pacientes</Link></BreadcrumbLink></BreadcrumbItem>
            <BreadcrumbSeparator />
            <BreadcrumbItem><BreadcrumbPage>{patient.name}</BreadcrumbPage></BreadcrumbItem>
          </BreadcrumbList>
        </Breadcrumb>

        <div className="bg-card border border-border rounded-2xl p-6 mb-6">
          <div className="flex flex-col md:flex-row gap-6 items-start">
            <PatientAvatar name={patient.name} src={patient.avatar_url ?? undefined} size={96} className="text-3xl" />
            <div className="flex-1 min-w-0 w-full">
              <div className="flex flex-wrap items-center gap-3">
                <h1 className="font-display text-3xl truncate">{patient.name}</h1>
                <Select value={patient.status} onValueChange={(v) => updateStatusMut.mutate(v as PatientStatus)}>
                  <SelectTrigger className={cn("h-7 w-[130px] border-0 text-xs font-medium", STATUS_META[patient.status].cls)}>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    {(Object.keys(STATUS_META) as PatientStatus[]).map((s) => (
                      <SelectItem key={s} value={s} className="focus:bg-muted/60 focus:text-foreground">{STATUS_META[s].label}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
                {patient.is_minor && (
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-4 mt-5">
                ...
                <InfoField icon={<Calendar className="h-3.5 w-3.5" />} label="Nascimento"
                  value={patient.birth_date ? `${new Date(patient.birth_date).toLocaleDateString("pt-BR")}${age !== null ? ` · ${age} anos` : ""}` : "—"} />
                <InfoField icon={<UserIcon className="h-3.5 w-3.5" />} label="Gênero"
                  value={patient.gender ? (GENDER_LABEL[patient.gender] ?? patient.gender) : "—"} />
                <InfoField icon={<Clock className="h-3.5 w-3.5" />} label="Última sessão"
                  value={patient.last_session ? new Date(patient.last_session).toLocaleDateString("pt-BR") : "—"} />
              </div>
              ...
              {patient.is_minor && (
                <div className="mt-5 border border-border rounded-xl p-4 bg-muted/30">
                  ...
                  <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-4">
                    <InfoField icon={<UserIcon className="h-3.5 w-3.5" />} label="Nome" value={patient.guardian_name ?? "—"} />
                    <InfoField icon={<Mail className="h-3.5 w-3.5" />} label="E-mail" value={patient.guardian_email ?? "—"} />
                    <InfoField icon={<Phone className="h-3.5 w-3.5" />} label="WhatsApp" value={patient.guardian_phone ?? "—"} />
                  </div>
                </div>
              )}
        ...
        <Tabs value={tab} onValueChange={setTab}>
          ...
          <TabsContent value="atendimentos" className="space-y-6 mt-0">
            <section className="bg-card border border-border rounded-2xl p-6">
              ...
              {upcoming ? (
                <div className="flex items-center gap-4 flex-wrap">
                  ...
                  <span className={cn("text-xs px-2.5 py-1 rounded-full", PAY_META[upcoming.payment_status].cls)}>{PAY_META[upcoming.payment_status].label}</span>
                </div>
              ) : (
            ...
            <section className="bg-card border border-border rounded-2xl p-6">
              <h2 className="font-display text-lg mb-4">Anotações da sessão</h2>
              <RichTextEditor value={draft} onChange={setDraft} placeholder="Comece a escrever..." onReady={setEditor} />
          ...
          <TabsContent value="historico" className="mt-0">
            <section className="bg-card border border-border rounded-2xl p-6">
              ...
              {history.length === 0 ? (
                <EmptyState icon={FileSearch} title="Nenhuma sessão registrada" description="As sessões concluídas com anotações aparecerão aqui." />
              ) : (
                <div className="relative pl-6">
                  <div className="absolute left-[7px] top-2 bottom-2 w-px bg-border" />
                  <div className="space-y-3">
                    {history.map((s) => <SessionTimelineItem key={s.id} session={s} />)}
                  </div>
                </div>
              )}
            </section>
          ...
          <TabsContent value="financeiro" className="space-y-6 mt-0">
            ...
            <section className="bg-card border border-border rounded-2xl overflow-hidden">
              ...
              {fin.totalCount === 0 ? (
                <EmptyState icon={CircleDollarSign} title="Sem movimentações" description="As sessões realizadas e seus pagamentos aparecerão aqui." />
              ) : (
                  ...
                  <tbody>
                    {patientSessions.filter((s) => s.status === "done").map((s) => (
                      <tr key={s.id} className="border-b border-border last:border-0">
                        ...
                        <td className="p-4 text-sm tabular-nums text-right">R$ {Number(s.value).toLocaleString("pt-BR")}</td>
                        <td className="p-4">
                          <Select value={s.payment_status} onValueChange={(v) => updateSessionMut.mutate({ sessionId: s.id, updates: { payment_status: v as PaymentStatus } })}>
                            ...
                            <SelectContent>
                              {(Object.keys(PAY_META) as PaymentStatus[]).map((k) => (
                                <SelectItem key={k} value={k} className="focus:bg-muted/60">{PAY_META[k].label}</SelectItem>
                              ))}
                            </SelectContent>
                          </Select>
                        </td>
      ...
      <ScheduleSessionDialog
        ...
        onCreate={(s) => {
          if (!user) return;
          insertSessionMut.mutate({ ...s, owner_id: user.id, patient_id: id }, {
            onSuccess: () => toast.success("Sessão agendada"),
          });
        }}
      />
      ...
      <EditPatientDialog
        ...
        patient={patient}
        onSave={(updated) => updatePatientMut.mutate(updated, { onSuccess: () => setOpenEdit(false) })}
      />
    </AppShell>
  );
}
...
function EditPatientDialog({
  ...
  patient: PatientRow;
  onSave: (p: Partial<PatientRow>) => void;
}) {
  const [form, setForm] = useState(patient);
  const upd = <K extends keyof PatientRow>(k: K, v: PatientRow[K]) => setForm((f) => ({ ...f, [k]: v }));
        ...
        <form onSubmit={submit} className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5">
              ...
              <Input type="date" value={form.birth_date ?? ""} onChange={(e) => upd("birth_date", e.target.value || null)} />
            </div>
            <div className="space-y-1.5">
              <Label>Gênero</Label>
              <Select value={form.gender ?? ""} onValueChange={(v) => upd("gender", v)}>
                ...
                <SelectContent>
                  {Object.entries(GENDER_LABEL).map(([k, v]) => <SelectItem key={k} value={k}>{v}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
            <div className="space-y-1.5">
            ...
            <div className="space-y-1.5 sm:col-span-2">
              ...
              <Textarea rows={3} value={form.notes ?? ""} onChange={(e) => upd("notes", e.target.value || null)} />
            </div>
          </div>

          <div className="flex items-center gap-2 pt-2">
            <Checkbox id="ep-minor" checked={!!form.is_minor} onCheckedChange={(v) => upd("is_minor", !!v)} />
          ...
          {form.is_minor && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 border border-border rounded-xl p-4 bg-muted/30">
              <div className="space-y-1.5 sm:col-span-2">
                ...
                <Input value={form.guardian_name ?? ""} onChange={(e) => upd("guardian_name", e.target.value || null)} />
              </div>
              <div className="space-y-1.5">
                <Label>E-mail do responsável</Label>
                <Input type="email" value={form.guardian_email ?? ""} onChange={(e) => upd("guardian_email", e.target.value || null)} />
              </div>
              <div className="space-y-1.5">
                <Label>WhatsApp do responsável</Label>
                <Input value={form.guardian_phone ?? ""} onChange={(e) => upd("guardian_phone", e.target.value || null)} />
              </div>
            </div>
          )}
...
function ScheduleSessionDialog({
  ...
  open: boolean;
  onOpenChange: (v: boolean) => void;
  onCreate: (s: Omit<Database["public"]["Tables"]["sessions"]["Insert"], "owner_id" | "patient_id">) => void;
}) {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    onCreate({
      date_time: new Date(`${date}T${time}:00`).toISOString(),
      ...
      <DialogContent className="max-w-md">
        ...
        <form onSubmit={submit} className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5"><Label>Duração (min)</Label><Input type="number" min={10} max={240} value={duration} onChange={(e) => setDuration(Number(e.target.value))} /></div>
            <div className="space-y-1.5"><Label>Valor (R$)</Label><Input type="number" min={0} value={value} onChange={(e) => setValue(Number(e.target.value))} /></div>
          </div>
          <DialogFooter>
            <Button type="button" variant="ghost" onClick={() => onOpenChange(false)}>Cancelar</Button>
...
function TabTrigger({ value, children }: { value: string; children: React.ReactNode }) {
  return (
    <TabsTrigger value={value} className="rounded-none border-b-2 border-transparent bg-transparent px-4 py-3 text-sm text-muted-foreground shadow-none data-[state=active]:bg-transparent data-[state=active]:border-primary data-[state=active]:text-primary data-[state=active]:shadow-none hover:text-foreground">
      {children}
    </TabsTrigger>
  );
}

function SessionTimelineItem({ session }: { session: SessionRow }) {
      ...
      <Collapsible open={open} onOpenChange={setOpen} className="border border-border rounded-xl hover:border-primary/40 transition-colors">
        <CollapsibleTrigger className="w-full text-left p-4">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-muted-foreground">{fmtTime(session.date_time)} · {session.duration_minutes} min</p>
            </div>
            <span className={cn("text-xs px-2.5 py-1 rounded-full", PAY_META[session.payment_status].cls)}>{PAY_META[session.payment_status].label}</span>
        ...
        <CollapsibleContent>
          <div className="px-4 pb-4 pt-0 border-t border-border mx-4 min-w-0">
            {session.notes ? (
              <div className="rt-content text-sm text-foreground pt-3 max-h-[320px] overflow-y-auto break-words [overflow-wrap:anywhere]" dangerouslySetInnerHTML={{ __html: session.notes }} />
            ) : (
...
function KpiCard({ label, value, sub, accent }: { label: string; value: string; sub: string; accent?: "success" | "warning" }) {
